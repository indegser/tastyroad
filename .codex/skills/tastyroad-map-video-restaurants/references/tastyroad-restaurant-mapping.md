...
```bash
python3 scripts/promote_verified_places.py --input data/verified_places/<file>.json
```

Apply agent artifacts:

```bash
python3 scripts/reduce_agent_artifacts.py --stage place_verification --apply
```

Validate public listing contract:

```bash
python3 scripts/verify_public_listing_contract.py
```
