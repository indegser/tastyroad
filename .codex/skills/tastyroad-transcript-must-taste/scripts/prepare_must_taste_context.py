    ...
    (pass_dir / "04_arbiter_result.md").write_text(
        "\n".join(
            [
                ...
                "- Final items must include candidate_id, reason, repaired_reason, quality, review, evidence, and optional supporting_evidence.",
                "- Build a source-fragment pack from evidence plus supporting_evidence in transcript order before writing final copy.",
                "- Choose reason with this source-window selector before running the editor. The selector picks raw material only; it does not repair ASR, polish text, or write public copy.",
                "- Selector prompt: 너는 맛집 영상 자막에서 공개용 추천 문구의 원문 재료를 고르는 편집자다. 목표는 보정문을 쓰는 것이 아니라, 보정하기 좋은 selected_raw_reason을 고르는 것이다.",
                "- Selector input is menu_item, existing reason, and context_fragments from evidence/supporting_evidence.",
                "- Pick only the subtitle span that best shows the menu's taste, recommendation, or distinctive feature.",
                "- Keep price, order transitions, background, or chatter only when they make that taste/recommendation quote work.",
                "- If the span is too short and loses the subject or target, attach a little surrounding context.",
                "- If the span is too wide and mixes several claims or background details, narrow it.",
                "- If a tail looks clipped but the next fragment moves to price, order, a new menu, background, or another claim, do not force it in. Leave the nearest raw span that the subtitle editor can trim naturally.",
                "- Do not fix ASR errors, spacing, or broken subtitle sentences in the selector. Do not summarize or create new wording. selected_raw_reason must be raw context_fragments joined as needed.",
                "- Use reason for the selected expanded raw transcript context: copy it from the selected source fragments in source order without ASR repair, summary, marketing rewrite, or artificial separators such as `/`.",
                "- Use repaired_reason for public display by running this flexible subtitle editor pass over reason.",
                "- Editor prompt: 너는 맛집 영상 자막을 공개용 짧은 인용문으로 다듬는 편집자다. 목표는 원문을 새로 쓰는 것이 아니라, 사람이 읽을 때 어색하지 않게 아주 가볍게 보정하는 것이다.",
                "- Input to the editor is menu_item and raw reason. Preserve the source tone, order, wording, reactions, menu words, and praise intensity as much as possible.",
                "- Fix only awkward subtitle breaks, spacing, repeated words, and obvious ASR/readability issues. Do not turn it into explanation, summary, ad copy, or analyst prose.",
                "- Do not add evaluations or conclusions that are not in the source. Do not force clipped fragments into complete sentences; keep only the naturally readable range.",
                "- This copy appears as a short quote in a restaurant list. Keep the parts that directly help the menu taste, recommendation, or distinctive quality; keep price/order setup/background only when they make the quote work.",
                "- Do not try to preserve every detail. If ASR is uncertain or a tail is awkward, do not guess; trim it away. If repeated conditions or connectors say the same thing, keep source wording and remove only awkward repetition.",
                "- A complete finite sentence is not required. If it reads naturally as a subtitle quote, keep it close to the source. When uncertain, edit less.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    task_path.write_text(
        "\n".join(
            [
                ...
                "- For each selected item, first choose source fragments from evidence/supporting_evidence in transcript order.",
                "- Choose reason with this source-window selector before running the editor. The selector picks raw material only; it does not repair ASR, polish text, or write public copy.",
                "- Selector prompt: 너는 맛집 영상 자막에서 공개용 추천 문구의 원문 재료를 고르는 편집자다. 목표는 보정문을 쓰는 것이 아니라, 보정하기 좋은 selected_raw_reason을 고르는 것이다.",
                "- Selector input is menu_item, existing reason, and context_fragments from evidence/supporting_evidence.",
                "- Pick only the subtitle span that best shows the menu's taste, recommendation, or distinctive feature.",
                ...
                "- If a tail looks clipped but the next fragment moves to price, order, a new menu, background, or another claim, do not force it in. Leave the nearest raw span that the subtitle editor can trim naturally.",
                "- Do not fix ASR errors, spacing, or broken subtitle sentences in the selector. Do not summarize or create new wording. selected_raw_reason must be raw context_fragments joined as needed.",
                "- Store reason as selected expanded raw transcript context copied from those source fragments with no newline; join fragments with ordinary spaces/punctuation, do not insert `/` separators, and do not repair ASR or summarize it.",
                "- Store repaired_reason as the public display copy by applying the subtitle editor prompt below to the raw reason.",
                "- Editor prompt: 너는 맛집 영상 자막을 공개용 짧은 인용문으로 다듬는 편집자다. 목표는 원문을 새로 쓰는 것이 아니라, 사람이 읽을 때 어색하지 않게 아주 가볍게 보정하는 것이다.",
                "- Preserve the source tone, order, wording, reactions, menu words, and praise intensity as much as possible.",
                "- Fix only awkward subtitle breaks, spacing, repeated words, and obvious ASR/readability issues. Do not turn it into explanation, summary, ad copy, or analyst prose.",
                "- Do not add evaluations or conclusions that are not in the source. Do not force clipped fragments into complete sentences; keep only the naturally readable range.",
                "- This copy appears as a short quote in a restaurant list. Keep the parts that directly help the menu taste, recommendation, or distinctive quality; keep price/order setup/background only when they make the quote work.",
                "- Do not try to preserve every detail. If ASR is uncertain or a tail is awkward, do not guess; trim it away. If repeated conditions or connectors say the same thing, keep source wording and remove only awkward repetition.",
                "- A complete finite sentence is not required. If it reads naturally as a subtitle quote, keep it close to the source. When uncertain, edit less.",
                "- Do not over-trim the source context; include enough surrounding words so the subject and claim remain understandable.",
                "",
